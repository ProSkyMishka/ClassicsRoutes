...
extension NetworkService {
    func getAllMessages(chat: ChatDate) async throws -> [MessageDate] {
        ...
        for message in messages {
            if chat.messages.contains(message.id) {
                let messageDate = MessageDate(id: message.id, user: message.user, route: message.route, time: Constants.format.date(from: message.time)!, text: message.text)
    ...
    func createMessage(user: String,
        ...
        let dto = MessageDTO(user: user, route: route, time: time, text: text)
        guard let url = URL(string: "\(localhost)\(APIMethod.getAllMessages.rawValue)")
        else {
            throw NetworkError.badURL
        }
